...
function longestZigzaggingSubsequence(seq) {
  let memo = [];
  seq.forEach((currNum, currIdx) => {
    // separate counts for zigzagging 'up' to this number and zigzagging 'down' to it
    let up = 0;
    let down = 0;
    seq.slice(0, currIdx).forEach((prevNum, prevIdx) => {
    ...
    // because we are zigzagging, record each value as opposite
    memo.push({ up: down + 1, down: up + 1  });
  });
